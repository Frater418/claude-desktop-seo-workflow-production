...
class ArtifactCandidateSaveRequest(BaseModel):
    ...
    primary_document: JsonValue
    supporting_documents: tuple[JsonValue, ...] = ()
...
def register_package4_routes(app: FastAPI, repository: ProjectRepository, provisioner: WorkspaceProvisioner, registry: WorkspaceRegistry) -> None:
    ...
    @app.post(f"{prefix}/artifacts/{{artifact_id}}/validate", response_model=DataEnvelope, operation_id="validateArtifactRevision")
    def artifact_validate(tenant_id: str, project_id: str, artifact_id: str, body: ArtifactValidationRequest) -> DataEnvelope:
        _assert_mutation_ready(app)
        return DataEnvelope(data=validate_artifact(repository, tenant_id, project_id, artifact_id, body).model_dump())
