...
<classification_policy>
  ...
  <market_deployment>Leite Markt, Sprache, Locale, Rechtsraum, Phase, Rolle und SEO-Betriebsmodell aus der beschriebenen geografischen und sprachlichen Reichweite ab. Binde jeden Einsatz exakt an einen vorhandenen market_registry-Eintrag. Providerwerte werden niemals erfunden.</market_deployment>
...
<instructions>
  ...
  <step number="3">Wende die field_resolution_policy und classification_policy auf alle Pflichtfelder an. Nutze dabei die vollstaendigen project_contracts und nur Eintraege aus market_registry.</step>
  ...
  <step number="5">Pruefe Querverweise zwischen Brand, Domains, Standorten, Service Areas, GBP-Profilen und Market Deployments. Pruefe vor einer vollstaendigen Ausgabe insbesondere: bediente Regionen sind nicht automatisch physical_locations; jedes lokale Betriebsmodell besitzt einen Standort- oder Servicegebietsbezug; jedes GBP-Profil verweist auf einen verifizierten physischen Standort. Erfinde keine lokale Praesenz, Evidence, Providerwerte oder Freigaben.</step>
  <step number="6">Wenn jedes Pflichtfeld direkt belegt, synthetisiert oder klassifiziert werden kann, liefere project_name, ein vollstaendiges Project-V2-Objekt und eine leere missing_fields-Liste.</step>
  <step number="7">Wenn mindestens ein echter fachlicher Pflichtwert missing bleibt, setze project_v2 auf null. Liste nur die konkreten, fuer einen Menschen beantwortbaren Informationsluecken auf Deutsch. Melde keine technischen IDs, Enum-Namen, Tenant-Werte, Autorenwerte, Zeitstempel oder Quellhashes.</step>
</instructions>
