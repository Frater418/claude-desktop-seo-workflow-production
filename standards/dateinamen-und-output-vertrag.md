...
| **2** | `2-cluster-recherche.xml.md` | `outputs/1-pillar-themen.md`, `standards/location-codes.json`, AgentSEO Tool-Call (`sync: false` plus `agentseo_job_status`) | `outputs/2-cluster-themen-agentseo.csv` (CSV) | Mindestens 25 validierte Zeilen pro Pillar, sonst `ERROR_INSUFFICIENT_CLUSTER_COVERAGE`. Suchvolumen und KD muessen verifiziert sein. Bei API-Quota-Fehler: Stopp mit Error-Code. |
| **3** | `3-120-tage-plan.xml.md` | `outputs/2-cluster-themen-agentseo.csv`, `capacity_matrix_solver.py` | `outputs/3-plan.md` (Markdown) | Horizont 17 Wochen, keine aktive Woche ueber 15.0 Stunden. Die gemessene Spanne steht im Plankopf, die Untergrenze von 10.0 Stunden prueft Gate 3 manuell. Lokale Pflicht-Landingpages zu 100% in Phase 1 und 2. |
| **3b** | `3b-performance-check.xml.md` | `outputs/3-plan.md`, `inputs/performance_export.csv` | `outputs/3b-performance-check.md` **und** ueberschriebene `outputs/3-plan.md` | Nur Inhalte aelter als 21 Tage werden bewertet. Kapazitaet der Folgephase bleibt exakt bei 10 bis 15h. |
