...
| 7 | **Tool-Klassifikation: Echte MCP-Tools vs. Prompt-Instruktionen**<br>Review schlaegt 8 Skills vor; nicht alle muessen eigene MCP-Server sein. | **Angepasst** | Recherche zu AgentSEO OpenAPI (`45 Endpunkte`, Stand 17.08.2026 sind 48 Tools im MCP-Server sichtbar) und Claude Desktop MCP Architektur (`stdio` Transport). | **Angepasst.** Differenzierung zwischen: (a) AgentSEO Remote-MCP Tools (`agentseo_*`), (b) Lokale Python/Helper Scripts (`capacity_matrix_solver.py`), und (c) Prompt-internen Validierungen. | Reduktion unnoetiger Server-Komplexitaet: 1 AgentSEO-MCP-Server + 1 Filesystem-Server + 1 lokaler deterministischer Solver, optional 1 GitHub-Server fuer Repo-Zugriffe. |
...
Betriebsbedingung aus dem Live-Test vom 17.08.2026: Jeder Aufruf uebergibt `location`, `location_code`, `language` und `sync: false` und holt das Ergebnis ueber `agentseo_job_status` ab. Details in ADR-008 und ADR-009.

### 3.4 Kapazitaets-Solver
Die Verteilung der Deliverables auf den Horizont von 17 Wochen mit einem Zielband von 10 bis 15 Arbeitsstunden bei variierenden Aufwaenden (Pillar: 8h, Blog: 3h, Landingpage: 1.25h, FAQ: 1h) unter Beruecksichtigung der Pflichtabdeckung fuer lokale Landingpages ist ein klassisches kombinatorisches Problem (Knapsack / Bin-Packing). Ein deterministisches Python-Script loest dies in Millisekunden fehlerfrei, waehrend LLMs dazu neigen, Wochen mit 8 oder 18 Stunden zu befuellen. Wichtig: der Solver erzwingt die Obergrenze von 15 Stunden, die Untergrenze von 10 Stunden ist ein Planungsziel und wird an Gate 3 geprueft. Belegt werden nur so viele Wochen, wie das Datenvolumen hergibt.
...
## 5. Nachtrag vom 17. August 2026

Der erste vollstaendige Lauf gegen den echten AgentSEO-Server hat drei der hier getroffenen
Entscheidungen praezisiert und eine Luecke offengelegt:

1. **Punkt 3 (AgentSEO):** Die Anbindung funktioniert, aber nur mit vollstaendigem Zielmarkt und
   asynchronem Abholmuster. Siehe ADR-008 und ADR-009.
2. **Punkt 4 (Solver):** Die Zusage "exakt 10 bis 15 Stunden pro Woche" war zu weit gefasst, der
   Code prueft nur die Obergrenze. Siehe ADR-004 Nachtrag.
3. **Punkt 8 (Human-in-the-Loop):** Die Gates greifen, aber Prosa-Regeln in Prompts wurden im Test
   gebrochen, waehrend Schema-Regeln hielten. Mengenregeln liegen deshalb jetzt im Manifest-Schema.
   Siehe ADR-010.

Die vollstaendige Liste der offenen Punkte steht in [CHANGELOG.md](../CHANGELOG.md) unter 1.3.0.
