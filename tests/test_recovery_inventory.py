...
class RecoveryInventoryTests(unittest.TestCase):
    ...
    def test_non_delivery_recovery_cannot_be_consumed_as_a_delivery_replay(self) -> None:
        families = ("artifact-recovery", "runtime-recovery", "transition-recovery")
        for family in families:
            with self.subTest(family=family), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                operator = root / "tenant-provisioned" / "project-provisioned" / "v2" / "operator"
                operator.mkdir(parents=True)
                (operator / "project.json").write_text(json.dumps({"tenant_id": "tenant-provisioned", "project_id": "project-provisioned"}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8")
                sidecar = operator / family / "recover.json"
                sidecar.parent.mkdir()
                sidecar.write_text("{}", encoding="utf-8")
                inventory = RecoveryInventory(ProvisionedWorkspaceResolver(WorkspaceRegistry(()), root, True))
                delivery_replay = RecoveryReplayIdentity("tenant-provisioned", "project-provisioned", "delivery-recovery", "delivery/recovery/idem-delivery-00000001.json")

                with self.assertRaisesRegex(RepositoryError, "recovery is pending"):
                    inventory.authorize(delivery_replay)

    def test_authorizes_only_the_exact_sole_delivery_sidecar(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            operator = root / "tenant-provisioned" / "project-provisioned" / "v2" / "operator"
            operator.mkdir(parents=True)
            (operator / "project.json").write_text(json.dumps({"tenant_id": "tenant-provisioned", "project_id": "project-provisioned"}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8")
            sidecar = operator / "delivery/recovery/idem-delivery-00000001.json"
            sidecar.parent.mkdir(parents=True)
            sidecar.write_text("{}", encoding="utf-8")
            inventory = RecoveryInventory(ProvisionedWorkspaceResolver(WorkspaceRegistry(()), root, True))
            replay = RecoveryReplayIdentity("tenant-provisioned", "project-provisioned", "delivery-recovery", "delivery/recovery/idem-delivery-00000001.json")

            self.assertEqual(replay, inventory.authorize(replay).replay)
            with self.assertRaisesRegex(RepositoryError, "recovery is pending"):
                inventory.authorize(RecoveryReplayIdentity("tenant-provisioned", "project-provisioned", "delivery-recovery", "delivery/recovery/idem-delivery-00000002.json"))
