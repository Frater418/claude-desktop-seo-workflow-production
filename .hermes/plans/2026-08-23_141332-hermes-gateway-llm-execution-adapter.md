...
**Status:** Completed minimal M08L cutline; broad backend platform deferred until after M10
**Created:** 2026-08-23
...
**Entry snapshot:** `wip/m08-output-quality-2026-08-23` at `568bb497e57af4f7ec6dc8a13438681bbf423a55`, remote SHA verified. Active Feature branch, HEAD and real Git index remained unchanged.
...
## 0. Binding Production-first override

Raphael approved the minimal adapter cutline on 2026-08-23. This section supersedes every broader pre-M10 implementation task later in this document.

M08L before M09 includes only:

1. Preserve the existing local fixture bytes, hashes and behavior with focused characterization tests.
2. Reuse the existing `ContextPackage`, `LLMRunRequest`, `LLMRunResult`, Worker Profile, output contracts, artifact revision, idempotency and runtime persistence authorities.
3. Add one thin Hermes Runs API client/backend for submit, bounded poll, terminal result, exact JSON extraction, model observation and token usage.
4. Add only the smallest explicit injection seam needed to select fixture or Hermes execution mode.
5. Validate the candidate through the existing output contracts before canonical persistence.
6. Record Hermes run ID, model, usage, timestamps and stable failures through the smallest existing runtime or diagnostic metadata seam.
7. Fail closed on auth, unavailable Gateway, timeout, failed, cancelled or interaction-required run, malformed JSON, contract mismatch, identity mismatch and idempotency conflict.
8. Run only the focused fixture regression, client/backend, exact runtime consumer, direct persistence/recovery and one neutral real-run closure.
9. Stop for Hermes independent acceptance before M09.

Explicitly deferred until after M10:

- general backend registry and model marketplace router
- direct OpenAI, Anthropic and Google adapters
- delegation and subagent contracts
- separate append-only execution-record persistence subsystem
- broad model benchmark framework
- speculative provider extension points and generalized recovery machinery

The broader tasks and schemas later in this document remain design reference only. They are not authorized before M10 unless a concrete blocker proves that one exact field or seam is indispensable to the minimal cutline.
...
- shared OAuth-pool boundary verification, with interactive Raphael reauthorization only if the Hermes-managed credential is unavailable
...
- isolated built-in `MEMORY.md` enabled
- `USER.md` and personal user-profile injection disabled
- external memory provider unset, including Hindsight
- no unrelated skills
...
```text
Hermes global auth store owns and refreshes the OpenAI-Codex OAuth credential
-> heartweb-runtime receives read-only provider-slice fallback through Hermes credential resolution
...
This is shared credential resolution, not a copied token file and not shared agent context. The profile retains isolated config, sessions, skills, built-in `MEMORY.md`, state database and logs. Credential writes remain profile-scoped; the global OAuth store is a read-only fallback for a provider that the profile has not configured locally.
...
Do not copy OAuth token files from the default profile. Do not pass OAuth access tokens, refresh tokens, authorization codes or device codes to root Sisyphus or any worker. If the Hermes-managed shared credential is unavailable or expired, M08L stops with `ERROR_LLM_BACKEND_AUTH` and Raphael reauthorizes through Hermes outside the agent run.
...
## 8. Broad task graph retained as post-M10 design reference

Only portions explicitly required by Section 0 may execute before M09. The remaining tasks in this section are deferred.
...
Minimal pre-M09 adapter cutline:

- preserve fixture regression and add smallest seam: 0.5h to 1.5h
- Hermes Runs client and exact JSON extraction: 0.5h to 1.5h
- existing validation and persistence integration: 0.5h to 1.0h
- focused failures and neutral real proof: 0.5h to 1.0h

Total minimal cutline: approximately 2h to 5h focused engineering time, excluding a real OAuth or Gateway blocker.

The former 13h to 26h generalized backend-platform scope is deferred until after M10 and requires a new Raphael authorization.
