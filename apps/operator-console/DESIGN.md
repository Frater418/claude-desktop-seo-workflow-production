...
- **Shell slot**: the application shell owns one named route-action slot outside the workspace scroll owner. Active workspaces portal only compact action controls into that slot while retaining forms, evidence, blockers, consequences, and detailed readback in their normal body flow. A standalone workspace without the slot renders the same controls inline for focused tests.
- **Layout**: desktop shell rows are header, scrollable workspace, then the action slot. The slot is omitted with no reserved space when the active route has no actions. At narrow widths it remains in normal layout flow and sticky at the viewport edge with safe-area inset padding, so it stays available without covering workspace content.
