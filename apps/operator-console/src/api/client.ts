...
type OperatorApiErrorOptions = { readonly kind: "http" | "network" | "unparseable"; readonly status: number; readonly message: string }
...
export type OperatorApiClient = {
  ...
  readonly validateArtifactRevision: (projectId: string, artifactId: string, request: ArtifactValidationRequest, signal: AbortSignal) => Promise<ArtifactValidationRead>
...
export function createOperatorApiClient(config: OperatorApiClientConfig): OperatorApiClient {
  ...
  return {
    ...
    validateArtifactRevision: async (projectId, artifactId, body, signal) => parseArtifactValidation(await postJson(config.baseUrl, `${projectPath(config.tenantId, projectId)}/artifacts/${encodeURIComponent(artifactId)}/validate`, body, signal)),
