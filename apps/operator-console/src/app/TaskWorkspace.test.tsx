...
import taskStyles from "../styles.css?inline"
...
const tasks: readonly TaskRead[] = [
  task({ taskId: "task-low", title: "Niedrige Nacharbeit", status: "resolved", owner: "Dora", priority: "low", deadline: "2026-08-26", stepId: "2", resolution: "Nachweis ablegen", dependency: "Keine" }),
  task({ taskId: "task-high", title: "Hohe Rueckfrage", status: "waiting_for_input", owner: "Bernd", priority: "high", deadline: "2026-08-22", stepId: "1", resolution: "Rueckfrage beantworten", dependency: "Kundenfreigabe" }),
  task({ taskId: "task-critical", title: "Kritische Freigabe", status: "open", owner: "Anna", priority: "critical", deadline: "2026-08-20", stepId: "0", resolution: "Freigabe erteilen", dependency: "Pruefbericht" }),
  task({ taskId: "task-medium", title: "Mittlere Pruefung", status: "open", owner: "Clara", priority: "medium", deadline: "2026-08-24", stepId: "1b", resolution: "Struktur pruefen", dependency: "Themeninventar" }),
]

afterEach(cleanup)

function task(overrides: Partial<TaskRead>): TaskRead {
  return { tenantId: currentRun.tenant_id, projectId: currentRun.project_id, runId: currentRun.run_id, stepId: currentRun.step_id, taskId: "task", title: "Aufgabe", status: "open", owner: "Anna", priority: "medium", deadline: "2026-08-20", resolution: "Loesung", dependency: "Keine", ...overrides }
}
...
describe("TaskWorkspace queue", () => {
  it("Given the deadline filter, when the operator opens it, then its German ISO-date guidance is deterministic", () => {
    render(<TaskWorkspace data={workspace(tasks)} />)

    const deadline = screen.getByLabelText("Faellig bis")
    expect(deadline).toHaveAttribute("placeholder", "JJJJ-MM-TT")
    expect(deadline).toHaveAttribute("inputmode", "numeric")
    expect(deadline).not.toHaveAttribute("type", "date")
    expect(screen.queryByText("mm/dd/yyyy")).toBeNull()
  })

  it("Given an invalid nonempty deadline, when the operator enters it, then the queue stays unfiltered and explains the correction", () => {
    render(<TaskWorkspace data={workspace(tasks)} />)
  ...
  it("Given a valid deadline filter, when it is cleared, then the full queue returns without an error", () => {
    render(<TaskWorkspace data={workspace(tasks)} />)
  ...
  it.each([
    ["Status filtern", "waiting_for_input", "Hohe Rueckfrage", "Rueckfrage beantworten", "Kundenfreigabe"],
  ...
  it("keeps a flexible title and intact deadline inside semantic task metadata", () => {
    ...
    const taskRow = screen.getByRole("button", { name: "Kritische Freigabe" })
    expect(taskRow.children).toHaveLength(2)
    expect(taskRow).toHaveAttribute("aria-label", "Kritische Freigabe")
    expect(taskRow).toHaveAttribute("aria-pressed", "true")
    expect(screen.getByRole("button", { name: "Hohe Rueckfrage" })).toHaveAttribute("aria-pressed", "false")
    expect(taskRow.querySelector(":scope > .task-row-title")).toHaveTextContent("Kritische Freigabe")
    expect(taskRow.querySelector(":scope > .task-row-metadata")).toHaveTextContent("Kritisch")
    expect(taskRow.querySelector(":scope > .task-row-metadata")).toHaveTextContent("Schritt 0")
    expect(taskRow.querySelector("time.task-row-date")).toHaveAttribute("dateTime", "2026-08-20")
    expect(taskRow.querySelector("time.task-row-date")).toHaveTextContent("2026-08-20")
    expect(taskStyles).toMatch(/\.task-row\s*\{[^}]*display:\s*grid;[^}]*grid-template-columns:\s*minmax\(0, 1fr\) auto;/)
    expect(taskStyles).toMatch(/\.task-row-metadata\s*\{[^}]*display:\s*flex;[^}]*flex-wrap:\s*wrap;/)
    expect(taskStyles).toMatch(/\.task-row-date\s*\{[^}]*white-space:\s*nowrap;[^}]*font-variant-numeric:\s*tabular-nums;/)
  })
})
