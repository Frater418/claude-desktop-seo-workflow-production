...
function fixture(options: FixtureOptions = {}): { readonly api: ReturnType<typeof createOperatorApiClient>; readonly calls: readonly string[] } {
  ...
  const fetch: typeof globalThis.fetch = (input) => {
    ...
    const current = { tenant_id: tenantId, project_id: selectedProjectId, run_id: runId, step_id: "1b", expected_revision: 17 }
    ...
    if (url.endsWith("/tasks")) return Promise.resolve(json({ data: [{ ...current, task_id: "aufgabe-welle-zwei", title: "Themenstruktur pruefen", status: "offen", owner: "Operator Welle Zwei", priority: "hoch", deadline: "2026-08-25", resolution: "Pillar-Struktur pruefen", dependency: "Freigabe" }] }))
    ...
    if (url.endsWith("/gates")) return Promise.resolve(json({ data: [{ ...current, run_id: options.gateRunId ?? runId, quality_gate_id: "GATE-1B", result: "passed", summary: "Maschinenpruefung bestanden" }] }))
    ...
    if (url.endsWith("/integrations/status")) return Promise.resolve(json({ data: [{ tenant_id: tenantId, project_id: selectedProjectId, name: "Notion", mode: "simuliert" }] }))
