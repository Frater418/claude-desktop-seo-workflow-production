...
function options(tasks: readonly TaskRead[], value: (task: TaskRead) => string): readonly string[] {
...
export function TaskWorkspace({ data }: { readonly data: OperatorWorkspaceData }): JSX.Element {
  ...
  const tasks = useMemo(() => sortedTasks(data.tasks.filter((task) => (filters.status === "" || task.status === filters.status) && (filters.owner === "" || task.owner === filters.owner) && (filters.priority === "" || task.priority === filters.priority) && (filters.deadline === "" || task.deadline <= filters.deadline) && (filters.step === "" || task.stepId === filters.step)), sortKey, sortDirection), [data.tasks, filters, sortDirection, sortKey])
  ...
  return <section className="task-layout">
    <section className="work-panel task-queue">
      <div className="work-heading"><div><p className="eyebrow">Aufgaben</p><h2>Aufgabenwarteschlange</h2></div><div className="action-row"><label>Status filtern<select aria-label="Status filtern" value={filters.status} onChange={(event) => updateFilter("status", event.currentTarget.value)}><option value="">Alle Status</option>{filterOptions.status.map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label>Verantwortung filtern<select aria-label="Verantwortung filtern" value={filters.owner} onChange={(event) => updateFilter("owner", event.currentTarget.value)}><option value="">Alle Verantwortlichen</option>{filterOptions.owner.map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label>Prioritaet filtern<select aria-label="Prioritaet filtern" value={filters.priority} onChange={(event) => updateFilter("priority", event.currentTarget.value)}><option value="">Alle Prioritaeten</option>{filterOptions.priority.map((value) => <option key={value} value={value}>{priorityLabel(value)}</option>)}</select></label><label>Faellig bis<input aria-label="Faellig bis" type="date" value={filters.deadline} onChange={(event) => updateFilter("deadline", event.currentTarget.value)} /></label><label>Schritt filtern<select aria-label="Schritt filtern" value={filters.step} onChange={(event) => updateFilter("step", event.currentTarget.value)}><option value="">Alle Schritte</option>{filterOptions.step.map((value) => <option key={value} value={value}>{value}</option>)}</select></label></div></div>
